...
double read_nuc_en(char* Path)
{
   ...
   if (!nuc_ener.is_open()) {
     std::cout << "Error: input file nuc_ener cannot open"<< std::endl;
   }
   while(!nuc_ener.eof()){
      nuc_ener >> nuc_en;
   }
...
void read_T(int ao, Matrix& T_int,char* Path){
   ...
   if (!kin_en.is_open()) {
     std::cout << "Error: input file kin_en cannot open"<< std::endl;
   }
...
void read_S(int ao, Matrix& S,char* Path){
   ...
   if (!overlap.is_open()) {
     std::cout << "Error: input file overlap cannot open"<< std::endl;
   }
   for(int k=0;k<((ao)*(ao+1))/2;k++){
      overlap >> i;
      overlap >> j;
...
void read_v_nuc(int ao, Matrix& v_nuc, char* Path){
   ...
   if (!interaction.is_open()) {
     std::cout << "Error: input file v_nuc cannot open"<< std::endl;
   }
   for(int k=0;k<((ao)*(ao+1))/2;k++){
      interaction >> i;
      interaction >> j;
...
void read_v_int(int ao, Matrix& V, char* Path){
   ...
   interaction.open(name.c_str());
   if (!interaction.is_open()) {
     std::cout << "Error: input file interaction cannot open"<< std::endl;
   }
